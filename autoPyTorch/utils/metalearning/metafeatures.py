    ...
    while len(to_visit) > 0:
        ...
        if name in npy_metafeatures:
            if X_transformed is None:
                ...
                fill_value = int(np.nanmax(X)) + 1 if not sparse else 0
                transformer = ColumnTransformer([
                    ('categorical',  make_pipeline(Imputer(strategy='constant', fill_value=fill_value), OneHotEncoder(categories='auto', sparse=True)), [i for i, c in enumerate(categorical) if c]),
                    ('numeric', Imputer(strategy='mean'), [i for i, c in enumerate(categorical) if not c])
                ])
                transformer.fit(X)
